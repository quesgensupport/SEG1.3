---
title: "SEG Shiny Heatmap (version 1.5)"
...
Provide the background documentation and code for the Shiny SEG app. This is currently version 1.4. The major changes to this app have been documented [here in this Google Document](https://docs.google.com/document/d/1nxhKBsMASUIXQxFrjZFvynBa0P3zDyBbGn2sCYNtkkU/edit?usp=sharing). 

## News for Version 1.5.1!!

Major changes to this version include:

- The font in the colored tables has been changed so it is easier to read. 

- include values for `risk_cat_txt` in Risk Table 2

- `15-20` and `20+` to the iso range table.   

- add plain language interpretation of MARD2 table byond 'models indicate'  

- display first ten rows (instead of the option to view the entire data set) 
...
```{r 2.4.3_VanderbiltWithMetabolic-2018-06-11.xlsx}
...
binomialTable <- function(dataset) {
  ...
  # 4.5.1 create probability, p-value, df_size ---- ----  ---- ----
  prb <- 0.95
  p_value <- 0.05
  df_size <- nrow(dataset)

  # 4.5.2 create criterion ---- ---- ---- ----
  qbinom_table <- qbinom(
    p = p_value,
    size = df_size,
    prob = prb
  ...
  BinomialTest6 <- bind_cols(CompliantPairs, QbinomTable)
  # 4.5.3 add the test for acceptance text ---- ----  ---- ----
...
```{r ggsave_heat_map_1.0, results='hide', echo=FALSE}
